...
[JsonConverter(typeof(Message41Converter))]
public record Message41
{
    ...
    private Message41(Optional<Message> messageValue,
    ...
    public static Message41 Message(Message value) =>
        new(Optional<Message>.Some(value), default, default, default);

    public static Message41 Message1(Message1 value) =>
        new(default, Optional<Message1>.Some(value), default, default);

    public static Message41 Message2(Message2 value) =>
        new(default, default, Optional<Message2>.Some(value), default);

    public static Message41 Message3(Message3 value) =>
    ...
    public static implicit operator Message41(Message value) => Message(value);

    public static implicit operator Message41(Message1 value) => Message1(value);

    public static implicit operator Message41(Message2 value) => Message2(value);

    public static implicit operator Message41(Message3 value) => Message3(value);
}

file sealed class Message41Converter : JsonConverter<Message41>
{
    public override Message41 Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        using var doc = JsonDocument.ParseValue(ref reader);
        var root = doc.RootElement;
        if (JsonSerializer.TryDeserialize<Message>(root, options, out var messageValue))
        {
            return Message41.Message(messageValue);
        }
        if (JsonSerializer.TryDeserialize<Message1>(root, options, out var message1Value))
        {
            return Message41.Message1(message1Value);
        }
        if (JsonSerializer.TryDeserialize<Message2>(root, options, out var message2Value))
        {
            return Message41.Message2(message2Value);
        }
        if (JsonSerializer.TryDeserialize<Message3>(root, options, out var message3Value))
        {
            return Message41.Message3(message3Value);
        }
        throw new JsonException($"JSON does not match Message or Message1 or Message2 or Message3 schemas: {root.ToString()}");
    }

    public override void Write(Utf8JsonWriter writer, Message41 value, JsonSerializerOptions options)
    {
        ...
        {
            throw new JsonException($"{nameof(Message41)} contains no valid value to serialize.");
        }
    }
}
